...
`anchor-data-ingest-gate` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies ingest behavior through transition tables, with invalid-transition tests and no credentials or hosted services.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 5.
